...
export class WorkoutHeaderElement extends HTMLElement {
  ...
  view = html`
    <header>
      <a href="/app" class="app-name">Workout Tracker</a>
      <nav class=${($: any) => $.authenticated ? "logged-in" : "logged-out"}>
        <menu>
          <li class="when-signed-in">
            <span>Hi, ${($: any) => $.username || ""}!</span>
          </li>
          <li class="when-signed-in">
  ...
  constructor() {
    super();
    shadow(this)
      .styles(reset, WorkoutHeaderElement.styles)
  ...
  static styles = css`
    ...
    a { color: var(--color-text-header); }
    span { color: var(--color-text-header); font-size: 0.9rem; }
    button {
      background: none;
